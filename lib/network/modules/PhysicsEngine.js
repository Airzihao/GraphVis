...
var CommunitySpringSolver = require('./components/physics/CommunitySpringSolver').default;
var CommunityRepulsionSolver = require('./components/physics/CommunityRepulsionSolver').default;
var util = require('../../util');
var EndPoints = require('./components/edges/util/EndPoints').default; // for debugging with _drawForces()

//var CommunityZoneFinder = require('./CommunityZoneFinder').default
...
class PhysicsEngine {
  ...
  init() {
    ...
    if (this.options.solver === 'forceAtlas2Based') {
      ...
      this.gravitySolver = new CentralGravitySolver(this.body, this.physicsBody, options);
      //this.zoneFinder = new CommunityZoneFinder(this.body, this.physicsBody,options);
    }
    else { // barnesHut
   ...
   */
  // Find the community zone when position stabilized.
  stopSimulation(emit = true) {
    this.stabilized = true;
    if (emit === true) {
      // for(let i=0;i<this.body.nodeIndices.length;i++){
      //   console.log(this.body.nodes[this.body.nodeIndices[i]].id,this.body.nodes[this.body.nodeIndices[i]].x,this.body.nodes[this.body.nodeIndices[i]].y);
      // }
      //this.zoneFinder.solve()
